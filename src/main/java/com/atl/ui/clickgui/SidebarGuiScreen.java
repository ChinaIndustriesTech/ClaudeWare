...
public class SidebarGuiScreen extends GuiScreen {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        // 1. Draw HD Rounded Main Panel (Transparent)
        drawHDBox(panelX, panelY, panelWidth, panelHeight, 8, 0xCC101010, false);
        
        // 2. Draw HD Rounded Sidebar (Solid, Left Corners Only)
        ...
        if (selectedModule != null) {
            ...
            for (Setting s : selectedModule.settings) {
                if (s instanceof BooleanSetting) {
                    ...
                    mc.fontRendererObj.drawStringWithShadow(s.name, settingsPanelX, currentSettingY + 3, b.enabled ? themeColor : (hovered ? -1 : 0xFF777777));
                    currentSettingY += 18;
                    ...
                    Gui.drawRect(settingsPanelX + 5, currentSettingY + 12, settingsPanelX + 5 + sliderWidth, currentSettingY + 14, 0xFF333333);
                    Gui.drawRect(settingsPanelX + 5, currentSettingY + 12, settingsPanelX + 5 + (int) renderWidth, currentSettingY + 14, themeColor);
    ...
    private void drawHDBox(double x, double y, double w, double h, double radius, int color, boolean leftOnly) {
        ...
        if (leftOnly) {
            ...
            // Flat right edge bottom
            worldrenderer.pos(x + w, y + h, 0).color(r, g, b, a).endVertex();
        } else {
            // Top-Right Corner (going backwards like working path)
    ...
    protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
        ...
        if (selectedModule != null) {
            ...
            if (mouseX >= settingsPanelX && mouseX <= settingsPanelX + settingsPanelWidth && mouseY >= settingsPanelY && mouseY <= settingsPanelY + settingsPanelHeight) {
                ...
                for (Setting s : selectedModule.settings) {
                    if (s instanceof BooleanSetting) {
                        if (isHovered(settingsPanelX, currentSettingY, settingsPanelWidth, 15, mouseX, mouseY)) ((BooleanSetting) s).toggle();
                        currentSettingY += 18;
                    } else if (s instanceof NumberSetting) {
                        currentSettingY += 25;
                    } else if (s instanceof ModeSetting) {
