...
public class Eagle extends Module {
    ...
    public void onPlayerTick(TickEvent.PlayerTickEvent event) {
        ...
        if (canEagle) {
            ...
            if (isMovingBackwards && isLookingDown && holdingBlock) {
                ...
                // If we meet the requirements to "Eagle" but aren't currently over an edge,
                // ensure the sneak state matches the physical key OR is forced true if automated.
                KeyBinding.setKeyBindState(sneakKey, !onlySneaking.isEnabled() || physicalSneak);
            }
        } else {
            // Not in bridging mode, ensure vanilla behavior
    ...
    private void updateDelay() {
        // Use the configurable delay with a tiny 0-20ms jitter for better bypass
