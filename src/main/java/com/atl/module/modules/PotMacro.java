...
public class PotMacro extends Module {
    ...
    public void onTick(TickEvent.ClientTickEvent event) {
        ...
        // Replicating legitimate input requires a tiny gap between key presses.
        // We decrement our delay and only process logic when it hits zero.
        if (tickDelay > 0) tickDelay--;
        if (tickDelay <= 0) processState(mc);
    }
    ...
    private void processState(Minecraft mc) {
        switch (state) {
            ...
            case SWAP:
                ...
                // Wait 1 tick for the server to acknowledge the slot change
                state = State.THROW;
                tickDelay = 1;
                break;
            ...
            case THROW:
                ...
                // Wait 1 tick for the potion to actually leave the hand
                state = State.RESTORE;
                tickDelay = 1;
                break;
