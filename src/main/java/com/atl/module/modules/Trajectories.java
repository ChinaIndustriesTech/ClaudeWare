...
public class Trajectories extends Module {
        ...
        for (int i = 0; i < 100; i++) {
            ...
            for (Entity entity : entities) {
                if (entity instanceof EntityLivingBase && entity != mc.thePlayer) {
                    ...
                    // Check if the ray actually intersects the entity's hitbox
                    ...
                    if (intercept != null) {
                        ...
                        // Only count the hit if it's closer than a block hit in this segment
                        if (dist < currentDist) {
                            currentDist = dist;
                            hitEntity = entity;
                            landing = intercept; // Use entity's intercept for hitmarker side and position
                        }
                    }
                }
            }
            ...
            if (landing != null || hitEntity != null) {
                ...
                // Only turn the line end green if it hit a player
                float r = hitEntity != null ? 0.0f : 1.0f;
                float b = hitEntity != null ? 0.0f : 1.0f;
                worldrenderer.pos(posX - renderPosX, posY - renderPosY, posZ - renderPosZ).color(r, 1.0f, b, 1.0f).endVertex();
                break;
            }
            ...
            worldrenderer.pos(posX - renderPosX, posY - renderPosY, posZ - renderPosZ).color(1.0f, 1.0f, 1.0f, 1.0f).endVertex();
        }

        tessellator.draw();

        if (landing != null || hitEntity != null) {
            GlStateManager.color(hitEntity != null ? 0.0f : 1.0f, 1.0f, hitEntity != null ? 0.0f : 1.0f, 1.0f);
    ...
    private void drawHitMarker(double x, double y, double z, EnumFacing side) {
        ...
        if (side == null) {
            // Fallback for entities or unknown hits (3D Star)
            ...
            // Calculate a tiny offset to prevent z-fighting with the block face
            double off = 0.01;
            
            switch (side) {
                case UP:
                case DOWN:
                    // Horizontal X on the XZ plane
