...
public class AutoBlockIn extends Module {
    ...
    private void handleRotate(Minecraft mc) {
        ...
        // LEGIT FIX: Set force to 'false' to use smooth interpolation
        Claude.rotationManager.setRotation(rotations[0], rotations[1], 10, false);

        // Check if the RotationManager has finished the smooth rotation
        if (Claude.rotationManager.isRotated()) {
            state = State.PLACE;
            stateStartTime = System.currentTimeMillis();
        } else if (System.currentTimeMillis() - stateStartTime > 500) { // Increased timeout for smooth rots
            reset();
        }
    }
    ...
    public void onDisable() {
        if (rotationCaptured) {
            Minecraft mc = Minecraft.getMinecraft();
            if (mc.thePlayer != null) {
                // Return to original view smoothly
