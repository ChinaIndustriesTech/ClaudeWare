...
public class BlinkManager {
    ...
    public void onPacketSend(PacketEvent.Send event) {
        if (blinkModule == BlinkModules.NONE) return;
        Packet<?> packet = event.getPacket();

        // Always let keepalive and chat through
        if (packet instanceof C00PacketKeepAlive || packet instanceof C01PacketChatMessage) return;
        // Let confirm transaction through if queue is empty
        if (blinkedPackets.isEmpty() && packet instanceof C0FPacketConfirmTransaction) return;
    ...
    public boolean setBlinkState(boolean state, BlinkModules module) {
        ...
        if (state) {
            ...
            blinking = false;
            blinkModule = BlinkModules.NONE; // Set to NONE first so onPacketSend doesn't re-queue
